...
def create_app(model, config, model_name: str):
    ...
    async def list_models():
        return {
            "object": "list",
            "data": [
                {"id": model_name, "object": "model", "created": int(time.time()), "owned_by": "user"}
            ],
        }
...
async def complete_chat(engine, prompt: str, model_name: str, max_tokens: int, temperature: float, sampler_kwargs: dict):
    tokens = []
...
# ANCHOR: stream_chat
async def stream_chat(engine, prompt: str, model_name: str, max_tokens: int, temperature: float, sampler_kwargs: dict):
    completion_id = f"chatcmpl-{uuid.uuid4().hex[:12]}"
    created = int(time.time())

    yield "data: " + json.dumps({"id": completion_id, "object": "chat.completion.chunk", "created": created, "model": model_name, "choices": [{"index": 0, "delta": {"role": "assistant"}, "finish_reason": None}]}) + "\n\n"

    token_count = 0
    async for token in engine.generate(prompt, max_tokens=max_tokens, temperature=temperature, **sampler_kwargs):
        token_count += 1
        yield "data: " + json.dumps({"id": completion_id, "object": "chat.completion.chunk", "created": created, "model": model_name, "choices": [{"index": 0, "delta": {"content": token}, "finish_reason": None}]}) + "\n\n"

    yield "data: " + json.dumps({"id": completion_id, "object": "chat.completion.chunk", "created": created, "model": model_name, "choices": [{"index": 0, "delta": {}, "finish_reason": "stop"}]}) + "\n\n"
    yield "data: [DONE]\n\n"
